              ...
              <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
                <Panel title={editing ? "تعديل طالب" : "إضافة طالب"}>
                  <form action={saveStudent} className="space-y-3">
                    ...
                    <input className="field" name="code" placeholder="الكود" defaultValue={editing?.code} required />
                    <input className="field" name="password" placeholder="كلمة المرور" defaultValue={editing?.password ?? "123456"} required />
                    <input className="field" name="grade" placeholder="الصف" defaultValue={editing?.grade ?? "2 إعدادي"} required />
                    <input className="field" name="seatNumber" placeholder="رقم الجلوس اختياري" defaultValue={editing?.seatNumber} />
                    <input className="field" name="avatar" placeholder="رابط الصورة الشخصية" defaultValue={editing?.avatar} />
                ...
                <div className="space-y-4">
                  <div className="glass-panel flex items-center gap-3 rounded-[1.5rem] p-3">
                    ...
                    <input className="w-full bg-transparent outline-none" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="بحث سريع بالاسم أو الكود أو الصف" />
                  </div>
                  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    {filteredStudents.map((student) => (
                      <StudentCard
                        key={student.id}
                        student={student}
                        rank={ranked.findIndex((item) => item.id === student.id) + 1}
                        onBonus={(item) => openPoint(item, "bonus")}
                        onMinus={(item) => openPoint(item, "minus")}
                        onEdit={setEditing}
                        onDelete={(id) => setStudents((current) => current.filter((item) => item.id !== id))}
                      />
                    ))}
                  </div>
                </div>
